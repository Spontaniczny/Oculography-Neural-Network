...
def _convolutions(in_channels: int, out_channels: int) -> nn.Sequential:
...
class U_NET(nn.Module):
    ...
    def __init__(self, input_size: int) -> None:
        ...
        self.encoders = nn.ModuleList([
            _convolutions(1, 64),
            _convolutions(64, 128),
            _convolutions(128, 256),
            _convolutions(256, 512)
        ])

        self.pooling_layer = nn.MaxPool2d(2)


        self._contracting_path, self._expanding_path, self._final_size = self._compute_itermediate_sizes(input_size)

        self.cropping_layers = nn.ModuleList([
            _Crop(residual_size, expanded_size)
            for residual_size, expanded_size in zip(
                self._contracting_path[::-1],
        ...
        self.decoders = nn.ModuleList([
            _convolutions(512, 256),
            _convolutions(256, 128),
            _convolutions(128, 64)
        ])
        
        self.final_convolution = nn.Conv2d(64, 1, (1, 1))


    def _compute_itermediate_sizes(self, input_size: int):
        current_size = input_size
        contracting_path_sizes = []
        for _ in range(3):
            current_size -= 4
            contracting_path_sizes.append(current_size)
            current_size //= 2

        current_size-= 4

        expanding_path_sizes = []
        for _ in range(3):
            current_size *= 2
            expanding_path_sizes.append(current_size)
            current_size -= 4
            
        return contracting_path_sizes, expanding_path_sizes, current_size
